...
4. Keep secreted CI job (`mcp-e2e`) restricted to manual trusted runs only:
   - `workflow_dispatch`
...
cd mcp
pnpm test
AEGIS_PRIVATE_KEY="$PRIVATE_KEY" pnpm run test:e2e
```

## Protected MCP E2E Validation (GitHub Actions)

After secret rotation or workflow updates are merged to `main`:

1. Trigger CI on `main` with `workflow_dispatch`.
...
5. Re-run MCP E2E manually at least once with current secret endpoints.
